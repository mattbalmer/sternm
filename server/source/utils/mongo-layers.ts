function findAll(Model, opts?) {
  return (req, res) => {
    ...
    if (opts && opts.populate) {
      const populate = Array.isArray(opts.populate) ? opts.popualte : [opts.populate];
      populate.forEach(populateCmd => {
        query = query.populate(populateCmd);
      });
    }
