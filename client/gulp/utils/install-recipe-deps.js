...
function scrape(dir, tREQUESTs) {
  let filenames = fs.readdirSync(dir);

  if(tREQUESTs) {
    filenames = filenames
      .filter(filename => tREQUESTs.indexOf(filename.replace(/\.js/g, '')))
  }
...
function generateCommand(dir, tREQUESTs) {
  dir = path.join(process.cwd(), dir || '');

  let dependencies = scrape(dir, tREQUESTs);
